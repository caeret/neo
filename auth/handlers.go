...
type BasicAuthFunc func(c *neo.Context, username, password string) (Identity, error)
...
func Basic(fn BasicAuthFunc, realm ...string) neo.Handler {
	name := DefaultRealm
	if len(realm) > 0 {
		name = realm[0]
	}
	return func(c *neo.Context) error {
		...
		return neo.NewHTTPError(http.StatusUnauthorized, e.Error())
	}
}
...
type TokenAuthFunc func(c *neo.Context, token string) (Identity, error)
...
// When authentication fails, a "WWW-Authenticate" header will be sent, and an http.StatusUnauthorized
// error will be returned.
func Bearer(fn TokenAuthFunc, realm ...string) neo.Handler {
	name := DefaultRealm
	if len(realm) > 0 {
		name = realm[0]
	}
...
func Query(fn TokenAuthFunc, tokenName ...string) neo.Handler {
	name := TokenName
	if len(tokenName) > 0 {
		name = tokenName[0]
	}
	return func(c *neo.Context) error {
		...
		if err != nil {
			return neo.NewHTTPError(http.StatusUnauthorized, err.Error())
		}
		c.Set(User, identity)
		return nil
	}
...
type JWTTokenHandler func(*neo.Context, *jwt.Token) error

// VerificationKeyHandler represents a handler function that gets a dynamic VerificationKey
type VerificationKeyHandler func(*neo.Context) string
...
func DefaultJWTTokenHandler(c *neo.Context, token *jwt.Token) error {
	c.Set("JWT", token)
	return nil
}
...
//	  })
//	}
func JWT(verificationKey string, options ...JWTOptions) neo.Handler {
	var opt JWTOptions
	if len(options) > 0 {
		opt = options[0]
	}
	...
	return func(c *neo.Context) error {
		...
		if message != "" {
			return neo.NewHTTPError(http.StatusUnauthorized, message)
		}
		return neo.NewHTTPError(http.StatusUnauthorized)
	}
}

...
type ServerOptions struct {
	...
	Allow func(*neo.Context, string) bool
}
...
//	     "/js": "/ui/dist/js",
//	}))
func Server(pathMap PathMap, opts ...ServerOptions) neo.Handler {
	...
	return func(c *neo.Context) error {
		if c.Request.Method != "GET" && c.Request.Method != "HEAD" {
			return neo.NewHTTPError(http.StatusMethodNotAllowed)
		}
		path, found := matchPath(c.Request.URL.Path, from, to)
		if !found || options.Allow != nil && !options.Allow(c, path) {
			return neo.NewHTTPError(http.StatusNotFound)
		}
...
func serveFile(c *neo.Context, dir http.Dir, path string) error {
...
func Content(path string) neo.Handler {

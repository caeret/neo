...
func TestContent(t *testing.T) {
	...
	if assert.NotNil(t, err) {
		assert.Equal(t, http.StatusMethodNotAllowed, err.(neo.HTTPError).StatusCode())
	}
	...
	err = h(c)
	if assert.NotNil(t, err) {
		assert.Equal(t, http.StatusNotFound, err.(neo.HTTPError).StatusCode())
	}
...
func TestServer(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.status == 0 {
			...
			if assert.NotNil(t, err, test.id) {
				assert.Equal(t, test.status, err.(neo.HTTPError).StatusCode(), test.id)
			}
		}
	}
	...
	h = Server(PathMap{"/css": "/testdata/css"}, ServerOptions{
		...
		Allow: func(c *neo.Context, path string) bool {
